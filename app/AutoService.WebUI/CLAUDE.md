...
- `StatusPillBadge` (`src/components/common/StatusPillBadge.tsx`) is the shared status-pill shell (geometry, dot, focus/typography); Scheduler's `StatusBadge` and Quotes' `QuoteStatusBadge` render through it while keeping their own status-to-color/dot maps local. `compactChipActionBaseClass` (`utils/styles/buttonStyles.ts`) is the shared compact chip-button base consumed by Customers' `customerCompactActionStyles.ts` and Scheduler's `schedulerButtonStyles.ts`, each of which still owns its local color/state variants. `monoIdentifierTextClass` (`utils/styles/textStyles.ts`) is the shared monospaced identifier style for plates/codes. `toolbarRowLayoutClass` / `toolbarActionsWrapperClass`, `compactRowHeaderClass` / `compactRowActionsClusterClass`, `centeredLoadingWrapperClass`, `compactInlineClusterClass`, `compactStackedRowClass`, and `rowHoverMotionClass` (all in `utils/styles/surfaceStyles.ts`) are the shared toolbar/compact-row/loading/motion layout primitives reused across Customers, Inventory, Quotes, Scheduler, Admin, CompanyResults, Settings, and the sidebar.
