...
- **Auth endpoints**:
  ...
  - `POST /api/auth/login` failure semantics: generic `401 invalid_credentials` for unknown identifier, wrong password, and when a linked mechanic domain record is missing (no 500 leak for domain-record gap); `403 mechanic_only_login` for existing customer email/phone identifiers; `429` lockout/rate-limit.
