...
- Catalog E2E coverage in `tests/e2e/catalog-parts.spec.ts` and `tests/e2e/catalog-labor-types.spec.ts`: tab switch reflected in the URL, toolbar search and sort, the live gross-price preview on create, reloading the server-returned row after edit, and the duplicate part-number/code conflict.
