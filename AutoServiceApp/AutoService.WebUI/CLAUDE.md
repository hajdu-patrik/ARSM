...
- Login and settings phone placeholders explicitly show EU-form examples (login: `e.g. +36 30 123 4567`; settings: `e.g. +36 30 123 4567 or +49 151 23456789`).
...
- `src/components/layout/SidebarLayout.tsx` — collapsible sidebar layout (fixed-width icon column for consistent alignment, smooth cubic-bezier width transition, mobile drawer, desktop collapse/expand without icon resizing); admin users see the admin nav item first in the main nav order (`admin` then default items), while non-admin users keep Scheduler-first ordering
